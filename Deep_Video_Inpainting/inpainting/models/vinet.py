...
class VINet_final(nn.Module):
    def __init__(self, opt):
        ...
        if self.opt.prev_warp:
            ...
            from Deep_Video_Inpainting.inpainting.lib.resample2d_package.resample2d import Resample2d
    ...
    def forward(self, masked_img, mask, prev_state=None, prev_feed=None, idx=0):
        ...
        # ==== SCALE - 1/4 : 64 ====
        flow2_64 = F.interpolate(flow2, scale_factor=2, mode='bilinear')*2
        flow3_64 = F.interpolate(flow3, scale_factor=2, mode='bilinear')*2
        flow4_64 = F.interpolate(flow4, scale_factor=2, mode='bilinear')*2
        flow5_64 = F.interpolate(flow5, scale_factor=2, mode='bilinear')*2
        flow6_64 = F.interpolate(flow6, scale_factor=2, mode='bilinear')*2
        ...
        # ==== SCALE - 1/2 : 128 ====
        flow2_128 = F.interpolate(flow2_64, scale_factor=2, mode='bilinear') * 2
        flow3_128 = F.interpolate(flow3_64, scale_factor=2, mode='bilinear') * 2
        flow4_128 = F.interpolate(flow4_64, scale_factor=2, mode='bilinear') * 2
        flow5_128 = F.interpolate(flow5_64, scale_factor=2, mode='bilinear') * 2
        flow6_128 = F.interpolate(flow6_64, scale_factor=2, mode='bilinear') * 2
        ...
        occ_mask = F.interpolate(occlusion_mask, scale_factor=8, mode='bilinear')
        occ_mask_64 = F.interpolate(occlusion_mask_64, scale_factor=4, 
                                 mode='bilinear')
        occ_mask_128 = F.interpolate(occlusion_mask_128, scale_factor=2, 
                                  mode='bilinear')

        flow6_256, flow6_512 = None, None
        if self.opt.prev_warp:
            if prev_state is not None or idx != 0:
                flow6_256 = F.interpolate(flow6_128, scale_factor=2, 
                                       mode='bilinear') * 2
                flow6_512 = F.interpolate(flow6_128, scale_factor=4, 
                                       mode='bilinear') * 4
                ...
                if self.opt.double_size:
                    ...
                    occlusion_mask_512 = F.interpolate(
